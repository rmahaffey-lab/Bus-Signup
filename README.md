...
<body>
    ...
    <div id="app" class="flex flex-col items-center justify-center min-h-screen p-4">
        <div id="loading" class="text-center text-gray-600">
            <h1 class="text-3xl font-bold text-gray-800">Loading Sign-Up Sheet...</h1>
        ...
        <div id="main-content" class="hidden w-full container mx-auto">
            <div id="auth-ui" class="flex flex-col items-center justify-center min-h-screen">
                <div class="card p-8 w-full max-w-sm text-center">
                    ...
                    <p class="text-sm text-gray-500 mb-6">Please sign in with your school email.</p>
                    <button id="signInBtn" class="w-full bg-blue-600 text-white font-semibold py-2 px-4 rounded-md hover:bg-blue-700 transition duration-300">
                        Sign in with Google
                    </button>
    ...
    <script type="module">
        ...
        import { getFirestore, doc, onSnapshot, collection, setDoc, FieldValue } from "https://www.gstatic.com/firebasejs/11.6.1/firebase-firestore.js";

        // Your web app's Firebase configuration
        // For Firebase JS SDK v7.20.0 and later, measurementId is optional
        const YOUR_FIREBASE_CONFIG = {
          apiKey: "AIzaSyChx1hUc5JhsNSvFCJqCFo4pAeUAbJC-O0",
          authDomain: "staff-route-sign-up.firebaseapp.com",
          projectId: "staff-route-sign-up",
          storageBucket: "staff-route-sign-up.firebasestorage.app",
          messagingSenderId: "583733533491",
          appId: "1:583733533491:web:0419cc007b0c7a262abc46",
          measurementId: "G-MD7GNJ10GB"
        };
        
        // Use provided config if available, otherwise use the hard-coded one
        ...
        async function initializeFirebase() {
            try {
                ...
                // Sign in with the custom token provided by the Canvas environment, or anonymously otherwise.
                if (initialAuthToken) {
                ...
                onAuthStateChanged(auth, async (user) => {
                    if (user) {
                        // User is signed in.
                        userId = user.uid;
                        // Display user info, showing ID for anonymous users
                        const userInfoText = user.isAnonymous
                            ? `Signed in as anonymous user (ID: ${userId})`
                            : `Signed in as ${user.email} (ID: ${userId})`;
        ...
        function renderBusStops(containerId, data, staffId, runType) {
            ...
            busStops.forEach(stop => {
                ...
                const currentSignUps = Object.keys(busData).length;
                const isSignedUp = busData[staffId] !== undefined;
                const isFull = currentSignUps >= 14;
                ...
                occupancy.textContent = `${currentSignUps} / 14 seats`;
                ...
                if (isSignedUp) {
                    ...
                    button.onclick = () => addSignUp(runType, docId, staffId);
                }
        ...
        async function addSignUp(runType, docId, staffId) {
            try {
                if (!db || !auth || !auth.currentUser) return;
                const docRef = doc(db, `artifacts/${appId}/public/data/${runType}/${docId}`);
                const data = { [staffId]: auth.currentUser.email };
        ...
        async function removeSignUp(runType, docId, staffId) {
            try {
                if (!db || !auth || !auth.currentUser) return;
                const docRef = doc(db, `artifacts/${appId}/public/data/${runType}/${docId}`);
                const data = { [staffId]: FieldValue.delete() };
                await setDoc(docRef, data, { merge: true });
            } catch (e) {
